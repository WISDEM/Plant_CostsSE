...
from twister.assemblies.openwind_assembly import openwind_assembly
...
class lcoe_cst_bos_ecn_ow_assembly(Assembly):
    ...
    windSpeed50m = Float(9.52, units = 'm/s', iotype='in', desc='mean annual wind speed at 50 m height')
    weibullK= Float(2.10, iotype='in', desc = 'weibull shape factor for annual wind speed distribution')
    ...
    def configure(self):
        super(lcoe_cst_bos_ecn_ow_assembly, self).configure()
        ...
        self.add('aep', openwind_assembly())
        ...
        # turbine configuration
        # drivetrain
        self.connect('ratedPower', ['tcc.ratedPower', 'bos.ratedPower', 'opex.ratedPower', 'fin.ratedPower', 'aep.ratedPower'])
        ...
        # plant configuration
        # climate
        #self.connect('shearExponent',['aep.shearExponent']) # unnecessary for openwind
        ...
        #self.connect('arrayLosses', ['aep.arrayLosses']) # not needed for openwind connection
        self.connect('turbineNumber', ['bos.turbineNumber', 'opex.turbineNumber', 'fin.turbineNumber', 'aep.turbineNumber'])
        ...
        self.connect('tcc.rotorDiameter', ['bos.rotorDiameter', 'aep.rotorDiameter'])
        self.connect('tcc.hubHeight', ['bos.hubHeight']) # , 'aep.hubHeight' # todo: hub height can't be adjusted at present
        ...
        self.create_passthrough('tcc.rpm')
    ...
    def execute(self):
        ...
        super(lcoe_cst_bos_ecn_ow_assembly, self).execute()  # will actually run the workflow


if __name__=="__main__":

    lcoe = lcoe_cst_bos_ecn_ow_assembly()
    ...
    lcoe.aep.ow.scriptfile = 'C:/Python27/openmdao-0.7.0/twister/models/AEP/VA_ECap.xml'
    ...
    f.write("Array AEP: |{0}\n".format(lcoe.array_aep))
    f.write("Array losses: |{0}\n".format(lcoe.array_losses))
