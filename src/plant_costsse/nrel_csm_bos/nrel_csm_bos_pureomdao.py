...
class bos_csm_component(Component):
    ...
    def __init__(self):
        ...
        # Variables
        self.add_param('machine_rating', val=0.0) # = Float(iotype='in', units='kW', desc='turbine machine rating')
        self.add_param('rotor_diameter', val=0.0) # =Float(iotype='in', units='m', desc='rotor diameter')
        ...
        self.add_output('bos_breakdown:development_costs', val=0.0) #  = Float(desc='Overall wind plant balance of station/system costs up to point of comissioning')
        self.add_output('bos_breakdown:preparation_and_staging_costs', val=0.0) #  = Float(desc='Site preparation and staging')
        self.add_output('bos_breakdown:transportation_costs', val=0.0) #  = Float(desc='Any transportation costs to site / staging site') #BOS or turbine cost?
        self.add_output('bos_breakdown:foundation_and_substructure_costs', val=0.0) # Float(desc='Foundation and substructure costs')
        self.add_output('bos_breakdown:electrical_costs', val=0.0) # Float(desc='Collection system, substation, transmission and interconnect costs')
        self.add_output('bos_breakdown:assembly_and_installation_costs', val=0.0) # Float(desc='Assembly and installation costs')
        self.add_output('bos_breakdown:soft_costs', val=0.0) # = Float(desc='Contingencies, bonds, reserves, decommissioning, profits, and construction financing costs')
        self.add_output('bos_breakdown:other_costs', val=0.0) # = Float(desc='Bucket for any other costs not captured above')

    def solve_nonlinear(self, params, unknowns, resids):

        self.machine_rating = params['machine_rating'] # = Float(iotype='in', units='kW', desc='turbine machine rating')
        self.rotor_diameter = params['rotor_diameter'] # =Float(iotype='in', units='m', desc='rotor diameter')
        ...
        self.year = params['year'] # = Int(2009, iotype='in', desc='year for project start')
        self.month = params['month'] # = Int(12, iotype = 'in', desc= 'month for project start')
        self.multiplier = params['multiplier'] # = Float(1.0, iotype='in')
        ...
        # Outputs
        unknowns['bos_costs'] = self.bos_costs #  = Float(iotype='out', desc='Overall wind plant balance of station/system costs up to point of comissioning')
        #self.add_output(bos_breakdown = VarTree(BOSVarTree(), iotype='out', desc='BOS cost breakdown')
        unknowns['bos_breakdown:development_costs'] = self.bos_breakdown_development_costs #  = Float(desc='Overall wind plant balance of station/system costs up to point of comissioning')
        unknowns['bos_breakdown:preparation_and_staging_costs'] = self.bos_breakdown_preparation_and_staging_costs #  = Float(desc='Site preparation and staging')
        unknowns['bos_breakdown:transportation_costs'] = self.bos_breakdown_transportation_costs #  = Float(desc='Any transportation costs to site / staging site') #BOS or turbine cost?
        unknowns['bos_breakdown:foundation_and_substructure_costs'] = self.bos_breakdown_foundation_and_substructure_costs # Float(desc='Foundation and substructure costs')
        unknowns['bos_breakdown:electrical_costs'] = self.bos_breakdown_electrical_costs # Float(desc='Collection system, substation, transmission and interconnect costs')
        unknowns['bos_breakdown:assembly_and_installation_costs'] = self.bos_breakdown_assembly_and_installation_costs # Float(desc='Assembly and installation costs')
        unknowns['bos_breakdown:soft_costs'] = self.bos_breakdown_soft_costs  # = Float(desc='Contingencies, bonds, reserves, decommissioning, profits, and construction financing costs')
        unknowns['bos_breakdown:other_costs'] = self.bos_breakdown_other_costs # = Float(desc='Bucket for any other costs not captured above')
